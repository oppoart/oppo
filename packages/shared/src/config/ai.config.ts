 ...
 * Centralized AI and ML service settings
 */
...
export function createAiConfig(): AiConfig {
  ...
  return {
    openai: {
      apiKey: openaiKey || '',
      model: process.env.AI_MODEL_PRIMARY || AI_MODELS.OPENAI.GPT4,
      maxTokens: parseInt(process.env.AI_MAX_TOKENS || String(TOKEN_LIMITS.DEFAULT)),
      temperature: TEMPERATURE_PRESETS.BALANCED,
    },
    anthropic: anthropicKey ? {
      apiKey: anthropicKey,
      model: AI_MODELS.ANTHROPIC.CLAUDE_3_SONNET,
      maxTokens: TOKEN_LIMITS.CLAUDE_3,
    } : undefined,
    fallbackModel: process.env.AI_MODEL_FALLBACK || AI_MODELS.OPENAI.GPT35_TURBO,
    rateLimit: parseInt(process.env.AI_RATE_LIMIT || String(AI_RATE_LIMITS.REQUESTS_PER_MINUTE.DEFAULT)),
  };
}
