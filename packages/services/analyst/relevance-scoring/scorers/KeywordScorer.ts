...
export interface KeywordMatch {
  ...
  weight: number;
  occurrences: number;
  field: string; // which field the match was found in
}
...
export interface KeywordAnalysis {
  ...
  coverage: number; // percentage of profile keywords found
}

export class KeywordScorer {
  private config: KeywordScorerConfig;
  ...
  async analyzeKeywordMatches(
    ...
    for (const [category, keywords] of Object.entries(profileKeywords)) {
      const categoryWeight = this.keywordWeights[category as keyof typeof this.keywordWeights] || 1.0;
      
      for (const keyword of keywords) {
        for (const [field, text] of Object.entries(opportunityTexts)) {
          const fieldWeight = this.fieldWeights[field as keyof typeof this.fieldWeights] || 1.0;
   ...
   */
  private extractProfileKeywords(profile: ArtistProfile): Record<string, string[]> {
   ...
   */
  private extractOpportunityTexts(opportunity: OpportunityData): Record<string, string> {
    return {
   ...
   */
  private countOccurrences(keyword: string, text: string): number {
   ...
   */
  private normalizeScore(weightedScore: number, totalMatches: number): number {
   ...
   */
  getTopMatches(analysis: KeywordAnalysis, limit: number = 10): KeywordMatch[] {
