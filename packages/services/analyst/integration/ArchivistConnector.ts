...
export interface StorageResult {
  ...
  duplicates: number;
  errors: string[];
  storedOpportunities: string[]; // IDs of successfully stored opportunities
}

export class ArchivistConnector {
  private config: ArchivistConnectorConfig;
  private archivistService?: ArchivistService;
  private isInitialized: boolean = false;

  constructor(
    private prisma: PrismaClient,
    config: Partial<ArchivistConnectorConfig> = {}
  ) {
    this.config = {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!this.archivistService) {
        this.archivistService = new ArchivistService(this.prisma);
      ...
      throw new AIServiceError(
        `Failed to initialize ArchivistConnector: ${error}`,
        'archivist',
   ...
   */
  async storeOpportunities(opportunities: OpportunityData[]): Promise<StorageResult> {
    ...
    try {
      ...
      const filteredOpportunities = this.config.enableFiltering ? 
        opportunities.filter(opp => 
          !opp.relevanceScore || opp.relevanceScore >= this.config.minRelevanceThreshold
        ) : opportunities;
      ...
      throw new AIServiceError(
        `Failed to store opportunities: ${error}`,
        'archivist',
        'opportunity-storage',
   ...
   */
  async getOpportunities(opportunityIds: string[]): Promise<Opportunity[]> {
    if (!this.isInitialized || !this.archivistService) {
      throw new Error('ArchivistConnector is not initialized');
    }
    ...
    try {
      ...
      throw new AIServiceError(
        `Failed to retrieve opportunities: ${error}`,
        'archivist',
   ...
   */
  async updateRelevanceScores(scores: Map<string, number>): Promise<{
    updated: number;
    errors: string[];
  }> {
    if (!this.isInitialized || !this.archivistService) {
    ...
    try {
      ...
      throw new AIServiceError(
        `Failed to update relevance scores: ${error}`,
        'archivist',
        'score-update'
      );
    }
   ...
   */
  async getHighRelevanceOpportunities(
    profileId: string,
    ...
    try {
      ...
      throw new AIServiceError(
        `Failed to get high relevance opportunities: ${error}`,
        'archivist',
        'high-relevance-query',
        { profileId, threshold, limit }
      );
    }
  }
  ...
  async searchOpportunities(filters: {
    ...
    try {
      ...
      throw new AIServiceError(
        `Failed to search opportunities: ${error}`,
        'archivist',
        'opportunity-search',
        { filters }
      );
   ...
   */
  async getArchivistStats(): Promise<{
  ...
  async healthCheck(): Promise<boolean> {
    try {
      if (!this.archivistService) return false;
    ...
    return opportunities.filter(opp => {
      ...
      if (criteria.requiredFields) {
        for (const field of criteria.requiredFields) {
          if (!opp[field as keyof OpportunityData]) {
            return false;
          }
        }
      }
